...
svyplot(~bmi+fat_percentage, nhanes_all_svy, style="bubble")

# Fit survey-specific linear models for each gender
model_male <- svyglm(fat_percentage ~ bmi, design = subset(nhanes_all_svy, sex == "Male"))
model_female <- svyglm(fat_percentage ~ bmi, design = subset(nhanes_all_svy, sex == "Female"))
...
bmi_range <- seq(min(nhanes_all_svy$variables$bmi, na.rm = TRUE), max(nhanes_all_svy$variables$bmi, na.rm = TRUE), by = 0.1)
...
# Plotting
fig <- ggplot(data = nhanes_all_svy$variables, aes(x = bmi, y = fat_percentage, color = sex)) +
