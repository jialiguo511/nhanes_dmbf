...
fat_plot <- ggplot(fat_racesex, aes(y = race_eth, x = estimate, color = dm)) +
  geom_point() +
  geom_errorbarh(aes(xmin = CI_lower-0.2, xmax = CI_upper+0.2), height = 0.2) +
  geom_text(aes(label = value), vjust = -0.8, hjust = 0.4, size = 4, color = "black") +
  facet_grid(dm ~ sex, scales = "fixed", space = "fixed") +
...
ggarrange(bmi_plot,
          ...
          nrow = 1,
          ncol = 3) %>% 
  ggsave(.,filename=paste0(path_nhanes_dmbf_folder,"/figures/one dm group/forest plot of BMI, BF and visceral fat by dm, race and sex.jpg"),width=32,height = 7)
