...
subfat_year <- fig_df %>% dplyr::filter(variable == "Subcutaneous fat mass")

fig_subfat <- ggplot(data=subfat_year,aes(x=year,ymin=CI_lower,ymax=CI_upper,y=estimate,col=dm,fill=dm,group=dm)) +
  geom_path() + 
  geom_ribbon(alpha=0.5) +
  labs(x = "Year",
       y = "Subcutaneous Fat Mass (g)",
       color = "Diabetes Status",
       fill = "Diabetes Status") +
  theme_bw() + 
  theme(
...
ggarrange(fig_bmi,
          ...
          nrow = 2,
          ncol = 2) %>% 
  ggsave(.,filename=paste0(path_nhanes_dmbf_folder,"/figures/one dm group/trend of BMI, BF, viceral and subcutaneous fat by year.jpg"),width=12,height = 8)
...
ggarrange(fig_bmi,
          ...
          nrow = 1,
          ncol = 3) %>% 
  ggsave(.,filename=paste0(path_nhanes_dmbf_folder,"/figures/one dm group/trend of BMI, BF, viceral fat by year.jpg"),width=14,height = 6)
...
ggarrange(fig_bmi_male,
          ...
          nrow = 2,
          ncol = 3) %>% 
  ggsave(.,filename=paste0(path_nhanes_dmbf_folder,"/figures/one dm group/trend of BMI, BF and visceral fat by year and sex one dm group.jpg"),width=16,height = 8)
