...
nhanes_total_weight <- nhanes_total %>% 
  mutate(fat_available = case_when(!is.na(bmi) & !is.na(fat_percentage) ~ 1,
                                   TRUE ~ 0),
         bmi = case_when(fat_available == 1 ~ bmi,
                         TRUE ~ NA_integer_))

ltfu_equation <- fat_available ~ female + race_eth + age
ltfu_fat_model <- glm(ltfu_equation, data = nhanes_total_weight, family = "binomial")
nhanes_total_weight$prob_fat = predict(ltfu_fat_model,newdata=nhanes_total_weight,type="response")


nhanes_total_svy <- nhanes_total_weight %>%
  mutate(ipw_fat = case_when(fat_available == 1 ~ 1/prob_fat,
                              TRUE ~ 1/(1-prob_fat)),
         combine_weights = ipw_fat * nhanes2yweight) %>% 
...
saveRDS(nhanes_total_svy, paste0(path_nhanes_dmbf_folder, "/working/cleaned/dbw01_survery and ipw weighted sample.RDS"))
