...
for (i in 1:length(nhanes_svy_dfs)) {
  ...
  nhanes_total_svy <- df %>% 
    mutate(
      dm_year = case_when(
        year == "2011-2012" & dm == "non-diabetes" ~ "2011-2012 non-diabetes",
        year == "2013-2014" & dm == "non-diabetes" ~ "2013-2014 non-diabetes",
        year == "2015-2016" & dm == "non-diabetes" ~ "2015-2016 non-diabetes",
        year == "2017-2018" & dm == "non-diabetes" ~ "2017-2018 non-diabetes",
...
all_results <- bind_rows(
  ...
  rename(estimate = theta_D,
         CI_lower = L, 
         CI_upper = U) %>% 
  write_csv(., "analysis/dbw02d_descriptive characteristics by dm and year.csv")
