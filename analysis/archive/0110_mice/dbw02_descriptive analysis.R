...
library(mitools)  # For pooling MI datasets
library(srvyr)
library(gtsummary)
library(gt)
...
results_list <- lapply(nhanes_svy_dfs, function(df) {
 
  tbl_svysummary(
    data = df,
    by = dm,
    missing_text = "Missing",
    include = c("age", "dm_age", "dm", "gender", "race_eth", "fipr", "immigrant", "education", "marital", "insurance"),
    ...
    statistic = list(all_continuous() ~ c("{mean} ({sd})", "{N_miss}"),
                     all_categorical() ~ "{p}%",
                     all_dichotomous() ~ "{p}%"),
    digits = list(all_continuous() ~ 1,  # Setting 1 decimal
                  all_categorical() ~ 1,
                  all_dichotomous() ~ 1)
  ) %>% 
    add_n(statistic = "{N_obs}") %>%  # Updated to match function
    add_overall() %>%
    as_gt()
})

# Pool results across all imputed datasets
pooled_results <- MIcombine(results_list)

# Save the pooled table to a Word document
gt::gtsave(pooled_results, filename = "analysis/dbw02_descriptive characteristics by dm.docx")
