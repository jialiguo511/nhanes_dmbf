...
for (i in 1:length(nhanes_svy_dfs)) {
  ...
  bmi_mod <- svyglm(bmi ~ age + race_eth + dm_year_sex, design = nhanes_total_svy)
  ...
  fat_mod <- svyglm(fat_percentage ~ age + race_eth + dm_year_sex, design = nhanes_total_svy)
  fat_emm <- emmeans(
    object = fat_mod, 
    specs  = ~ dm_year_sex, 
    data   = nhanes_total_svy$variables  # <-- specify the underlying data
  ...
  visfat_mod <- svyglm(visceral_fat ~ age + race_eth + dm_year_sex, design = nhanes_total_svy)
  # Calculate marginal (adjusted) means of BMI by dm_sex
  visfat_emm <- emmeans(
    object = visfat_mod, 
    specs  = ~ dm_year_sex, 
...
all_results <- bind_rows(
  ...
  pool_ad(visfat_list) %>% select(dm_year_sex, theta_D, L, U) %>% 
  ...
  rename(estimate = theta_D,
         CI_lower = L, 
         CI_upper = U) %>% 
  write_csv(., "complete cases/analysis/dbwse03e_descriptive characteristics by dm, sex and year.csv")
