...
for (i in 1:length(nhanes_svy_dfs)) {
  ...
  bmi_mod <- svyglm(bmi ~ age + race_eth + sex_year, design = nhanes_total_svy)
  ...
  fat_mod <- svyglm(fat_percentage ~ age + race_eth + sex_year, design = nhanes_total_svy)
  fat_emm <- emmeans(
    object = fat_mod, 
    specs  = ~ sex_year, 
    data   = nhanes_total_svy$variables  # <-- specify the underlying data
  ...
  visfat_mod <- svyglm(visceral_fat ~ age + race_eth + sex_year, design = nhanes_total_svy)
  # Calculate marginal (adjusted) means of BMI by dm_sex
  visfat_emm <- emmeans(
    object = visfat_mod, 
    specs  = ~ sex_year, 
...
all_results <- bind_rows(
  ...
  pool_ad(visfat_list) %>% select(sex_year, theta_D, L, U) %>% 
  ...
  rename(estimate = theta_D,
         CI_lower = L, 
         CI_upper = U) %>% 
  write_csv(., "complete cases/analysis/dbwse03f_descriptive characteristics by sex and year.csv")
