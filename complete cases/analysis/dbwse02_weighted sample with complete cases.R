...
mi_dfs <- readRDS(paste0(path_nhanes_dmbf_folder, "/working/cleaned/mi_dfs_cca.RDS"))
...
for(i in 1:mi_dfs$m) {
  ...
  # self-reported diagnosed DM, N = 362
  nhanes_dm_diag <- analytic_df %>% 
    dplyr::filter(dm_doc_told == 1) 
  
  
  # Identify undiagnosed DM based on A1c. Set dm_age = current age, N = 195
  ...
  # preDM, N = 2,268
  nhanes_predm <- analytic_df %>% 
    dplyr::filter(dm_doc_told == 0 &
                    (glycohemoglobin >= 5.7 & glycohemoglobin < 6.5) |
                    (fasting_glucose >= 100 & fasting_glucose < 126)) 
  
  # Total sample, N = obs = 9,910
  nhanes_total <- analytic_df %>%
    mutate(dm = case_when(
      respondentid %in% nhanes_dm_diag$respondentid ~ "DM",
      respondentid %in% nhanes_dm_undiag$respondentid ~ "DM",
      respondentid %in% nhanes_predm$respondentid ~ "PreDM",
      TRUE ~ "NoDM")) %>% 
    mutate(
...
saveRDS(nhanes_svy_dfs, paste0(path_nhanes_dmbf_folder, "/working/cleaned/dbwse02_weighted df with complete cases.RDS"))
